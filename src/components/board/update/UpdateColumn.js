...
const UpdateColumn = (props) => {
    ...
    useEffect(async() => {
        let response = await fetch(`/api/boardColumn/${props.match.params.boardId}/update/${cookies.userKey}`);
    ...
    const handleClickUpdate = async() => {
        if(!value.sportNo || !value.boardTitle){
            ...
            await axios.post(`/api/boardColumn/${value.boardNo}/update`, formData, config).then((response)=>{
...
export default withRouter(UpdateColumn);